...
#define DMibTupleTemplate NMib::NContainer::TCTuple
...
namespace NMib
{
	namespace NContainer
	{
			...
			, private NPrivate::TCTupleImp<typename NMeta::TCMakeConsecutiveIndices<sizeof...(t_PCTypes)>::CType, t_PCTypes...>
		{
			...
			friend fg_Get(tf_CType &&_Tuple) noexcept;
			...
			friend fg_GetForward(tf_CType &&_Tuple) noexcept;
		...
		{
			typedef typename TCTuple_Get<tf_Index, typename NPrivate::TCIsTuple<tf_CType &&>::CType>::CType CType;
			return fg_Forward<typename NTraits::TCCopyQualifiersAndReference<tf_CType &&, CType>::CType >
				(
					static_cast
					<
						typename NTraits::TCCopyQualifiersAndReference<tf_CType &&, NPrivate::TCTupleLeaf<tf_Index, CType>>::CType
					>(_Tuple).f_Get()
				)
			;
		}
		...
		namespace NPrivate
		{
			...
			struct TCMakeTupleReturnHelper0<NReference::TCReference<t_CType, t_CPtr>>
			{
				typedef t_CType& CType;
			};
